...
public class GuiBrainStoneTrigger extends GuiBrainStoneBase {
  ...
  public GuiBrainStoneTrigger(InventoryPlayer inventoryplayer, TileEntityBrainStoneTrigger te) {
    ...
    hovered = -1;
    max_page =
        (BrainStone.getClientSideTiggerEntities().size() / 4)
  ...
  public void initGui() {
    ...
    for (int i = 0; i < 4; i++) {
      buttons.addButton(
          new BrainStoneButton(i + 10, 8, 8, 12, 12 + (i * 18), 0, 166, 0, 174)
              .changeTriggerBox(7, 7 + (i * 18), 78, 24 + (i * 18)));
      ...
      for (int j = 0; j < 15; j++) {
        buttons.addButton(
            new BrainStoneButton((i * 20) + 32 + j, 2, 3, 88 + (2 * j), 13 + (18 * i), true));
      }
    }
  ...
  protected void drawGuiForeground(int mouseX, int mouseY) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      drawString(
          message, 25, 12 + (18 * i), tileentity.getMobTriggered(mobs[i]) ? 0xffffff : 0x111111);
    }
    ...
    if ((hovered != -1) && tileentity.getMobTriggered(mobs[hovered])) {
      ...
      drawTexturedModalRect(
          88, 13 + (18 * hovered), 88, 166, tileentity.getMobPower(mobs[hovered]) * 2, 6);
    }
  ...
  private void setMobs() {
    ...
    final String[] keys =
        BrainStone.getClientSideTiggerEntities().keySet().toArray(new String[length]);
