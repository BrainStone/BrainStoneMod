...
public class BrainStoneClientConfigWrapper {
  ...
  @Getter private static boolean enableJEIReloading;
  @Getter private static boolean BSLCallowStealing;
  @Getter @JEIReload private static long BSLCRFperHalfHeart;

  /**
   * Set the override values.
   *
   * @param values The values from the server
   * @throws RuntimeException Throws an exception when the map contains a field it doesn't expect
   *     (using a sneaky throw from lombok) or if something else went terribly wrong.
   */
  @SneakyThrows
