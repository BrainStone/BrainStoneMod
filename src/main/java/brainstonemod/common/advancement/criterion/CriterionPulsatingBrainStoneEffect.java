...
public class CriterionPulsatingBrainStoneEffect
	...
	private final Map<PlayerAdvancements, CriterionPulsatingBrainStoneEffect.Listeners> listeners = Maps.newHashMap();
	...
	public void addListener(PlayerAdvancements playerAdvancementsIn,
		...
		if (upgradeBrainStoneLifeCapacitor$listeners == null) {
			upgradeBrainStoneLifeCapacitor$listeners = new CriterionPulsatingBrainStoneEffect.Listeners(
	...
	@Override
	public CriterionPulsatingBrainStoneEffect.Instance deserializeInstance(JsonObject json,
	...
	static class Listeners {
		...
		private final Collection<ICriterionTrigger.Listener<CriterionPulsatingBrainStoneEffect.Instance>> listeners = Sets
				.newHashSet();
		...
		public void add(ICriterionTrigger.Listener<CriterionPulsatingBrainStoneEffect.Instance> listener) {
			listeners.add(listener);
		}

		public void remove(ICriterionTrigger.Listener<CriterionPulsatingBrainStoneEffect.Instance> listener) {
			listeners.remove(listener);
		}

		public void trigger(EntityPlayerMP player, boolean protection) {
			List<ICriterionTrigger.Listener<CriterionPulsatingBrainStoneEffect.Instance>> toGrant = new LinkedList<>();

			for (ICriterionTrigger.Listener<CriterionPulsatingBrainStoneEffect.Instance> listener : listeners) {
				if (listener.getCriterionInstance().test(protection)) {
					toGrant.add(listener);
				}
			}

			for (ICriterionTrigger.Listener<CriterionPulsatingBrainStoneEffect.Instance> listener : toGrant) {
