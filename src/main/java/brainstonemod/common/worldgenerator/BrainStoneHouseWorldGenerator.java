...
public class BrainStoneHouseWorldGenerator implements IWorldGenerator {
  ...
  private static final ResourceLocation LOOT_TABLE_BOTTOM =
      new ResourceLocation(BrainStone.RESOURCE_PACKAGE, "chests/house/bottom");

  private static final ResourceLocation CHAMBER =
      new ResourceLocation(BrainStone.RESOURCE_PACKAGE, "house/chamber");
  private static final ResourceLocation REPLACEMENT_STONE =
      new ResourceLocation(BrainStone.RESOURCE_PACKAGE, "house/replacement_stone");
  private static final ResourceLocation REPLACEMENT_WOOD =
      new ResourceLocation(BrainStone.RESOURCE_PACKAGE, "house/replacement_wood");
  private static final ResourceLocation SHACK =
      new ResourceLocation(BrainStone.RESOURCE_PACKAGE, "house/shack");
  private static final ResourceLocation STARIS =
      new ResourceLocation(BrainStone.RESOURCE_PACKAGE, "house/stairs");
  private static final ResourceLocation STARIS_END =
      new ResourceLocation(BrainStone.RESOURCE_PACKAGE, "house/stairs_end");
  ...
  private void generate() {
    ...
    // Generate Shack
    Template templateReplacementWood =
        templateManager.getTemplate(minecraftServer, REPLACEMENT_WOOD);
    List<BlockPos> dataBlocks =
        templateShack.getDataBlocks(housePos, placementSettings).entrySet().stream()
            .filter(entry -> entry.getValue().equals("chest"))
  ...
  private void generateStairCase() {
    ...
    Template templateReplacementStone =
        templateManager.getTemplate(minecraftServer, REPLACEMENT_STONE);
    PlacementSettings placementSettings = (new PlacementSettings()).setRotation(rotation);
    ...
    BlockPos stairBase =
        templateStairs
            .getDataBlocks(new BlockPos(0, 0, 0), placementSettings)
            .entrySet()
            .iterator()
            .next()
            .getKey();
    BlockPos stairPos =
        basePos
            .subtract(stairBase)
            .add(
                random.nextInt(baseSize.getX() - 2) + 1,
                -1,
    ...
    stairPos =
        stairPos
            .add(0, -2, 0)
            .offset(
                rotation.rotate(EnumFacing.EAST),
    ...
    for (Entry<BlockPos, String> dataBlock :
        templateChamber.getDataBlocks(stairPos, placementSettings).entrySet()) {
