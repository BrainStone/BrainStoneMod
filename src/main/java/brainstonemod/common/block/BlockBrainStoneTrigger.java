...
public class BlockBrainStoneTrigger extends BlockBrainStoneHiders {
  ...
  public void breakBlock(World world, BlockPos pos, IBlockState state) {
    final TileEntityBrainStoneTrigger tileentityblockbrainstonetrigger =
        (TileEntityBrainStoneTrigger) world.getTileEntity(pos);
  ...
  public int getStrongPower(
    ...
    return ((tileentityblockbrainstonetrigger != null)
            && (tileentityblockbrainstonetrigger.getDelay() > 0))
  ...
  public boolean isSideSolid(
      IBlockState base_state, IBlockAccess world, BlockPos pos, EnumFacing side) {
    return true;
  }
  ...
  public boolean onBlockActivated(
    ...
    if (tileentityblockbrainstonetrigger != null) {
      playerIn.openGui(
          BrainStone.instance,
          BrainStoneGuiHandler.ID_BRAIN_STONE_TRIGGER,
          world,
          pos.getX(),
          pos.getY(),
          pos.getZ());
  ...
  public void onBlockAdded(World world, BlockPos pos, IBlockState state) {
    ...
    world.scheduleUpdate(
        pos, this, tickRate(world) - ((int) world.getTotalWorldTime() % tickRate(world)));
  }
   ...
   *
   * @param world The world. Needed to access the blocks
   * @param i x-coordinate
   * @param j y-coordinate
   * @param k z-coordinate
   * @return True if there is at least one mob to trigger on top of this block, false if not.
   */
  private byte triggerCorrectMob(World world, int i, int j, int k) {
    final List<?> list =
        world.getEntitiesWithinAABBExcludingEntity(
            null, new AxisAlignedBB(i, j + 1, k, i + 1, j + 2, k + 1));
    final TileEntityBrainStoneTrigger tileentityblockbrainstonetrigger =
        (TileEntityBrainStoneTrigger) world.getTileEntity(new BlockPos(i, j, k));

    if (tileentityblockbrainstonetrigger == null) return 0;
  ...
  public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
    ...
    tileEntityBlockBrainStoneTrigger.setOutput(
        triggerCorrectMob(world, pos.getX(), pos.getY(), pos.getZ()));
    if (tileEntityBlockBrainStoneTrigger.getOutput() > 0) {
      tileEntityBlockBrainStoneTrigger.setOutputBuffered(
          tileEntityBlockBrainStoneTrigger.getOutput());
    }

    tileEntityBlockBrainStoneTrigger.setDelay(
        (byte)
            ((tileEntityBlockBrainStoneTrigger.getOutput() > 0)
    ...
    if (tileEntityBlockBrainStoneTrigger.checkForSlotChange()) {
      BrainStonePacketHelper.sendReRenderBlockAtPacket(
          world.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), world);
    }
