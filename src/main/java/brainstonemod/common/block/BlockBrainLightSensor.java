...
public class BlockBrainLightSensor extends BlockBrainStoneContainerBase {
  ...
  public int getStrongPower(
    ...
    final TileEntityBrainLightSensor tileentity =
        (TileEntityBrainLightSensor) blockAccess.getTileEntity(pos);

    if (tileentity == null) return 0;

    if (tileentity.isClassic()) return tileentity.getPowerOn() ? 15 : 0;
    else return tileentity.getPower();
  }
  ...
  public boolean onBlockActivated(
      ...
      float hitZ) {
    final TileEntityBrainLightSensor tileentity =
        (TileEntityBrainLightSensor) world.getTileEntity(pos);
    ...
    if (tileentity == null) {
      ...
      playerIn.openGui(
          BrainStone.instance,
          BrainStoneGuiHandler.ID_BRAIN_LIGHT_SENSOR,
          world,
          pos.getX(),
          pos.getY(),
          pos.getZ());
    ...
    if (world.isRemote) {
      ...
      for (i = 0; i < randInt; i++) {
        world.spawnParticle(
            ...
            (x + (random.nextDouble() * 1.4)) - 0.2,
            y + 0.8 + (random.nextDouble() * 0.6),
            (z + (random.nextDouble() * 1.4)) - 0.2,
            0.0D,
            0.0D,
            0.0D);
      }
  ...
  public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
    ...
    if (tileEntity != null) {
      ...
      if (tileEntity.isClassic()) {
        ...
        tileEntity.setPowerOn(
            tileEntity.getDirection() ? worldLight <= lightLevel : worldLight >= lightLevel);
      } else {
