...
public class BlockPulsatingBrainStone extends Block {
  ...
  public int damageDropped(IBlockState state) {
    if (effect) return 0;
    else return getMetaFromState(state) & 0x01;
  }
  ...
  public static void applyEffect(World world, BlockPos pos, Random random) {
    ...
    final List<Entity> list =
        world.getEntitiesWithinAABBExcludingEntity(
            null, new AxisAlignedBB(pos.add(-10, -10, -10), pos.add(11, 11, 11)));
      ...
      if (tmpEntity instanceof EntityLivingBase) {
        ...
        if (entity.getDistance(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5) <= radius) {
          ...
          if (entity instanceof EntityPlayer) {
            CriterionRegistry.PULSATING_BRAIN_STONE_EFFECT.trigger(
                (EntityPlayerMP) entity, isProtected);
          }
  ...
  public static void potionEffect(EntityLivingBase entity, Random random) {
    ...
    entity.addPotionEffect(
        new PotionEffect(
            potion, potion.isInstant() ? 1 : (random.nextInt(5980) + 20), random.nextInt(4)));
  }
  ...
  public static boolean isProtected(Iterable<ItemStack> armorStacks) {
    ...
    if (armor.size() < 4) return false;
      ...
      offset = 1;
    } else if (armor.size() > 5) return false;

    if (armor.stream().skip(offset).anyMatch(ItemStack::isEmpty)) return false;
    ...
    return armor.stream()
        ...
        .allMatch(
            stack ->
                (stack.getItem() instanceof ItemArmorBrainStone)
