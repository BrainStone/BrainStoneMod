...
public class BlockBrainStoneAnvil extends BlockAnvil {
  ...
  public boolean onBlockActivated(
    ...
    if (!world.isRemote) {
      playerIn.openGui(
          BrainStone.MOD_ID,
          BrainStoneGuiHandler.ID_BRAIN_STONE_ANVIL,
          world,
          pos.getX(),
          pos.getY(),
          pos.getZ());
  ...
  public void updateTick(World world, BlockPos pos, IBlockState state, Random random) {
    if (effect) {
      if (!world.isRemote
          && (random.nextInt(BrainStoneConfigWrapper.getPulsatingBrainStoneJumpChance()) == 0)
        ...
        EntityFallingBlock entityfallingblock =
            new EntityFallingBlock(
                world, pos.getX() + 0.5D, pos.getY(), pos.getZ() + 0.5D, world.getBlockState(pos));
