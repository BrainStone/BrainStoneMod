 ...
 * This class provides a lot functionality around working with the current jar file.
 */
public final class BrainStoneJarUtils {
  ...
  /**
   * Extracts ONE certificate chain from the specified certificate array which may contain multiple
   * certificate chains, starting from index 'startIndex'.
   */
  ...
  private static boolean isJarSigned() {
    if (!RUNNING_FROM_JAR) return true;

    try {
      @Cleanup final JarFile jarFile = getRunningJar();
  ...
  private static void verify(X509Certificate targetCert) throws IOException {
    // Sanity checking
    if (targetCert == null) throw new SecurityException("Provider certificate is invalid");

    @Cleanup final JarFile jarFile = getRunningJar(true);
    ...
    while (entries.hasMoreElements()) {
      ...
      entriesVec.add(je);
      @Cleanup final InputStream is = jarFile.getInputStream(je);
  ...
  /**
   * Verifies the signature of the jar. Will do nothing if the jar is not signed
   *
   * @throws SecurityException This method throws an SecurityException if for any reason the
   *     signature is invalid.<br>
   *     <br>
   *     Possible reasons:
   *     <ul>
   *       <li>There are unsigned files in the jar
   *       <li>The Hash of a file doesn't match (meaning the file has been altered)
  ...
  public static void verifyJar() throws SecurityException {
    if (!SIGNED_JAR) return;

    final String certStr =
        "MIIFsTCCA5mgAwIBAgIECyArtTANBgkqhkiG9w0BAQsFADCBiDELMAkGA1UEBhMCREUxHDAaBgNV"
    ...
    try {
      ...
      @Cleanup final ByteArrayInputStream inStream = new ByteArrayInputStream(certBytes);
