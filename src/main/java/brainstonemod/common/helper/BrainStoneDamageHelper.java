...
public class BrainStoneDamageHelper {
  public static float getAdjustedDamage(
      DamageSource damageSource, float initalDamage, EntityPlayer player, boolean simluate) {
    float adjustedDamage = initalDamage;

    adjustedDamage =
        applyArmor(player, player.inventory.armorInventory, damageSource, adjustedDamage, simluate);
    if (adjustedDamage <= 0) return 0;
   ...
   * Taken from {@link net.minecraft.entity.EntityLivingBase#applyPotionDamageCalculations}
   */
  private static float applyPotionDamageCalculations(
      DamageSource damageSource, float initialDamage, EntityPlayer player) {
    if (damageSource.isDamageAbsolute()) return initialDamage;
    else {
      ...
      if (initialDamage <= 0.0F) return 0.0F;
        ...
        int k =
            EnchantmentHelper.getEnchantmentModifierDamage(
                player.getArmorInventoryList(), damageSource);
  ...
  /**
   * Gathers and applies armor reduction to damage being dealt to a entity.<br>
   * Taken from {@link net.minecraftforge.common.ISpecialArmor.ArmorProperties#applyArmor}. Added
   * simulate flag to prevent damaging of armor.
   *
  ...
  private static float applyArmor(
    ...
    double totalToughness =
        entity.getEntityAttribute(SharedMonsterAttributes.ARMOR_TOUGHNESS).getAttributeValue();
        ...
        if ((absorb > 0) && !simulate) {
          ...
          if (stack.getItem() instanceof ISpecialArmor) {
            ((ISpecialArmor) stack.getItem())
                .damageArmor(entity, stack, source, itemDamage, prop.Slot);
          } else {
    ...
    if ((damage > 0) && ((totalArmor > 0) || (totalToughness > 0))) {
      ...
      damage =
          CombatRules.getDamageAfterAbsorb(
              (float) damage, (float) totalArmor, (float) totalToughness);
    }
   ...
   * Taken from {@link net.minecraftforge.common.ISpecialArmor.ArmorProperties#StandardizeList}
   *
