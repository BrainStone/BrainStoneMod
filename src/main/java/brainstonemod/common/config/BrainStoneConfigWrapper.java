...
public class BrainStoneConfigWrapper {
  ...
  @Getter @ServerOverride private static boolean enableJEIReloading;
  ...
  private static void loadModuleSettings() {
    for (ModuleInformation module : BrainStoneModules.getAllModules()) {
      if (getBoolean(
          CAT_MODULES,
          "Disable" + firstUpper(module.getModid()),
          false,
          "Manually disable the compatibility module for " + module.getName(),
          true)) {
        if (module.isActive()) {
    ...
    enableJEIReloading =
        getBoolean(
            ...
            "EnableJEIReloading",
            true,
            "Reload JEI when settings change.\n"
  ...
  private static void loadMiscellaneousSettings() {
    enableCreativeTab =
        getBoolean(
            ...
            true);
    essenceOfLifeBaseChance =
        ((getInt(
                        CAT_MISC,
                        "EssenceOfLifeDropChanceModifier",
                        0,
                        -50,
                        75,
                    ...
                    / 100.0)
                + 1.0)
            * (BrainStoneModules.draconicEvolution() ? 0.1 : 0.5);
    pulsatingBrainStoneJumpChance =
        getInt(
            CAT_MISC,
            "PulsatingBrainStoneAnvilJumpChance",
            100,
            0,
            1000,
  ...
  private static void loadBrainStoneLifeCapacitorSettings() {
    ...
    addCustomCategoryComment(
        CAT_BSLC, "This set defines the behavior of the BrainStoneLifeCapacitor.");
  }
   ...
   * @param defaultValue Default value of the property.
   * @param comment A brief description what the property does.
   * @param requiresRestart Whether Minecraft requires a restart if this changed while running.
   * @return The value of the new boolean property.
   */
  private static boolean getBoolean(
      String category, String name, boolean defaultValue, String comment, boolean requiresRestart) {
