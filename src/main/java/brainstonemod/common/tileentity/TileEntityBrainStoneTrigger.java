...
public class TileEntityBrainStoneTrigger extends TileEntity implements IInventory {
  private static final ArrayList<TileEntityBrainStoneTrigger> failedTileEntities =
      new ArrayList<>();
  ...
  public void disableAllMobs() {
    mobTriggered.keySet().stream()
        .filter(mob -> mobTriggered.get(mob) > 0)
  ...
  public void readFromNBT(NBTTagCompound compound) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      try {
        ...
        try {
          mobTriggered.put(
              compound.getString(trigger + "Key"), compound.getBoolean(trigger) ? 15 : 1);
        } catch (final Exception e1) {
          try {
            mobTriggered.put(
                compound.getString(trigger + "Key"), (int) (compound.getByte(trigger)));
