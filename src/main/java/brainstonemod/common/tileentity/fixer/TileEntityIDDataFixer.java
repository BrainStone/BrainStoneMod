...
public class TileEntityIDDataFixer implements IFixableData {
	...
	public TileEntityIDDataFixer() {
		idsToFix = ImmutableMap.<String, String>builder()
				.put("minecraft:tileentityblockbrainlightsensor", BrainStone.RESOURCE_PREFIX + "brain_light_sensor")
				.put("minecraft:tileentityblockbrainstonetrigger", BrainStone.RESOURCE_PREFIX + "brain_stone_trigger")
				.build();
	}
	...
	public NBTTagCompound fixTagCompound(NBTTagCompound tag) {
		...
		if (idsToFix.containsKey(tileEntityID))
			tag.setString("id", idsToFix.get(tileEntityID));
