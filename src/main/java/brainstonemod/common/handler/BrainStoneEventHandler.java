...
public class BrainStoneEventHandler {
  ...
  public static ItemStack getBrainStoneLiveCapacitor(EntityPlayer player) {
    ...
    ItemBrainStoneLifeCapacitor.PlayerCapacitorMapping mapping =
        capacitor.getPlayerCapacitorMapping();
    UUID capacitorUUID = mapping.getCapacitorUUID(player.getUniqueID());

    if (capacitorUUID == null) return null;

    if (BrainStoneModules.baubles()) {
      ItemStack bStack =
          BaublesApi.getBaublesHandler(player).getStackInSlot(BaubleType.BELT.getValidSlots()[0]);

      if (checkStack(bStack, capacitorUUID)) return bStack;
    }

    for (ItemStack stack : player.inventory.mainInventory) {
      if (checkStack(stack, capacitorUUID)) return stack;
    }
  ...
  public void attachCapabilities(AttachCapabilitiesEvent<ItemStack> event) {
    if (event.getCapabilities().containsKey(BrainStone.RESOURCE_LOCATION)) return;
  ...
  public void onEntityAttack(LivingAttackEvent event) {
    if (event.getEntityLiving() instanceof EntityPlayer) {
      ...
      if (player.hurtResistantTime > (player.maxHurtResistantTime / 2.0f)) {
        if (amount <= player.lastDamage) return;
      ...
      if (capacitor != null) {
        float adjustedDamage =
            BrainStoneDamageHelper.getAdjustedDamage(event.getSource(), amount, player, true);
        float newDamage =
            BrainStoneItems.brainStoneLifeCapacitor().handleDamage(capacitor, adjustedDamage, true);
  ...
  public void onEntityDamage(LivingHurtEvent event) {
    if (event.getEntityLiving() instanceof EntityPlayer) {
      ...
      if (capacitor != null) {
        float newDamage =
            BrainStoneItems.brainStoneLifeCapacitor().handleDamage(capacitor, event.getAmount());
        ...
        event
            ...
            .add(
                new EntityItem(
                    ...
                    new ItemStack(BrainStoneItems.essenceOfLife())));
      }
    }
  }
  ...
  public void onPlayerJoinClient(final ClientConnectedToServerEvent event) {
    (new Thread() {
          ...
          public void run() {
            ...
            BSP.debug(
                "Calling onPlayerJoinClient for "
                    + FMLClientHandler.instance().getClientPlayerEntity().getDisplayNameString());

            BrainStone.onPlayerJoinClient(
                FMLClientHandler.instance().getClientPlayerEntity(), event);
          }
        })
        .start();
  }
  ...
  public void playerJoinServer(PlayerEvent.PlayerLoggedInEvent event) {
    BrainStoneItems.brainStoneLifeCapacitor()
        .getPlayerCapacitorMapping()
        .updateName(event.player.getUniqueID(), false);
    PacketDispatcher.sendToAll(
        new PacketCapacitorData(
